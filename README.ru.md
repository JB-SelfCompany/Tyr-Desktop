...
1. **Соберите новый бинарник** с помощью скриптов сборки:
   - **Windows**: Запустите `build-windows.bat`
   - **Linux**: Запустите `./build-linux.sh`

2. **Замените старый исполняемый файл** новым скомпилированным бинарником из `build/bin/`

3. **Удалите старую версию** бинарника

Ваша конфигурация, база данных и все настройки будут сохранены автоматически - они хранятся отдельно в директории конфигурации и не будут затронуты при замене исполняемого файла.
...
### Windows
- **Конфиг**: `%APPDATA%\Tyr\config.toml`
- **База данных**: `%APPDATA%\Tyr\yggmail.db`
- **Пароль**: Windows Credential Manager (сервис: `Tyr`, пользователь: `default`)

### Linux
- **Конфиг**: `~/.config/tyr/config.toml`
- **База данных**: `~/.config/tyr/yggmail.db`
- **Пароль**: GNOME Keyring / KWallet (сервис: `Tyr`, пользователь: `default`)
  - Запасной вариант: `~/.config/tyr/.password` (AES-зашифрован)
